                    ...
                    try {
                        if (fs.existsSync("./skyfiles/" + myid + ".json")) {
                            ...
                            if (hm[1]=="list") {
                                ...
                                var shoptext = "You have " + mysky["coins"][user.id] + " coins.\n\n"
                                for (var i=0;i<mykeys.length;i++) {
                                    shoptext = shoptext + mykeys[i] + ": " + myshop[mykeys[i]]
                                ...
                                if (hm.length==4) {
                                    ...
                                    if (hm[2] in myshop) {
                                        try {
                                            var cost = myshop[hm[2]] * parseInt(hm[3])
                                ...
                                if (hm.length==4) {
                                    ...
                                    if (hm[2] in buyoff) {
                                        if (hm[2] in mysky["inventory"][user.id]) {
                                            if (mysky["inventory"][user.id][hm[2]] >= parseInt(hm[3])) {
                                                mysky["inventory"][user.id][hm[2]] -= parseInt(hm[3])
                                                mysky["coins"][user.id] += (parseInt(hm[3]) * buyoff[hm[2]])
                                                jsonedit.save("./skyfiles/" + myid + ".json", mysky)
                                                channel.send("Sold!")
                                            } else {
                                    ...
                                    channel.send("Incorrect syntax: ..shop sell [item] [amount]\n\nReplace all spaces in item name with an underscore!")
                                }
                            }
                        } else {
