              ...
              if (fs.existsSync("./skyfiles/" + myid + ".json")) {
                ...
                if (hm[1]=="give") {
                  ...
                  if (hm.length==3) {
                    var yourid=hm[2]
                    yourid=sha1(yourid)
                    if (fs.existsSync("./skyfiles/" + yourid + ".json")) {
                      var thefile=jsonedit.load("./skyfiles/" + yourid + ".json")
                      thefile["spaceFree"]+=100
                      thefile["level"]+=1
                      jsonedit.save("./skyfiles/" + yourid + ".json", thefile)
                      channel.send("Island updated!")
                    } else {
                      channel.send("This file does not exist!")
                    }
                  }
